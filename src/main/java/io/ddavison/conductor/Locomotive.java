...
public class Locomotive {
	...
	public Locomotive() {
		...
		case SAFARI:
			...
			break;
		case HTMLUNIT: // If you are designing a regression system, HtmlUnit is
			// NOT recommended.
		...
		if (!isLocal)
			...
			try {
				driver = new RemoteWebDriver(new URL(configuration.hub()),
						capabilities); // just override the driver.
			} catch (Exception x) {
				System.err.println("Couldn't connect to hub: "
						+ configuration.hub());
				return;
			}
	...
	static {
		...
		if (System.getProperty("os.name").toLowerCase().contains("mac")) {
			System.setProperty("webdriver.chrome.driver",
					findFile("chromedriver.mac"));
			System.setProperty("phantomjs.binary.path",
					findFile("phantomjs.mac"));
			System.setProperty("webdriver.firefox.driver", "");
		} else if (System.getProperty("os.name").toLowerCase().contains("nix")
				|| System.getProperty("os.name").toLowerCase().contains("nux")
				|| System.getProperty("os.name").toLowerCase().contains("aix")) {
			System.setProperty("webdriver.chrome.driver",
					findFile("chromedriver.linux"));
			System.setProperty("phantomjs.binary.path",
					findFile("phantomjs.linux"));
			System.setProperty("webdriver.firefox.driver", "");
			...
			System.setProperty("phantomjs.binary.path",
					findFile("phantomjs.exe"));
			System.setProperty("webdriver.firefox.driver", "");
		} else {
			log.warn("Unknown operating system. ");
		}
	}

	static public String findFile(String filename) {
		String paths[] = { "", "bin/", "target/classes" }; // if you have
		// chromedriver
		// somewhere else on
		// the path, then
		// put it here.
	...
	public WebElement waitForElement(By by) {
		...
		while (size == 0) {
			...
			if (attempts == MAX_ATTEMPTS)
				fail(String.format("Could not find %s after %d seconds",
	 ...
	 *
	 * @param css
	 *            The css element to set the text of.
	 * @param text
	 ...
	 *
	 * @param css
	 *            The css selector for the checkbox
	...
	/**
	 * Switches to a frame based on the index it comes. (starting at
	 * <code>0</code>)
	 *
	 * @param index
	 *            The index of the frame in which it appears
	 * @return <code>AutomationTest</code> (for fluency)
	 */
	 ...
	 *
	 * @param css
	 *            The css element to validate the text of.
	 * @param text
	...
	public Locomotive validateTextNot(By by, String text) {
		...
		assertFalse(String.format("Text matches! [expected: %s] [actual: %s]",
				text, actual), text.equals(actual));
		return this;
	}
	 ...
	 *
	 * @param text
	 *            The text to ensure is on the page.
	 * @return <code>AutomationTest</code> (for fluency)
	 */
	 ...
	 *
	 * @param text
	 *            The text to ensure is not on the page.
	 * @return <code>AutomationTest</code> (for fluency)
	 */
	...
	public Locomotive validateUrl(String regex) {
		...
		assertTrue("Url does not match regex [" + regex + "] (actual is: \""
				+ driver.getCurrentUrl() + "\")", m.find());
		return this;
	}
	 ...
	 *
	 * @param condition
	 *            The condition that is expected to be false
	 * @return <code>AutomationTest</code> (for fluency)
	 */
	...
	/**
	 * Get a variable from the data warehouse.<br>
	 * <br>
